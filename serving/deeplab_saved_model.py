...
import serving.network as network
...
tf.app.flags.DEFINE_string('export_model_dir',
                           "./versions",
                           'Directory where the model exported files should '
                           'be placed.')
...
pre_trained_model_dir = os.path.join(log_folder,
                                     model_name,
                                     "train")
...
def main(_):
    ...
    with tf.Session(graph=tf.Graph()) as sess:
        ...
        # It receives an image and its dimensions and output the
        # segmentation mask
