...
tf.flags.DEFINE_string("training_data_file",
                       TRAININGSET_DIR, "Data source for the training data.")
...
def test_ann():
    ...
    test_data = feed.load_data_and_labels(FLAGS.test_data_file, FLAGS.num_classes,
    ...
    if BEST_OR_LATEST.upper() == 'B':
        ...
        checkpoint_file = checkpoints.get_best_checkpoint(FLAGS.best_checkpoint_dir, select_maximum_value=True)
    else:
    ...
    with graph.as_default():
        ...
        with sess.as_default():
            ...
            batches = feed.batch_iter(list(zip(x_test, y_test, y_test_labels)), FLAGS.batch_size, 1, shuffle=False)
            ...
            for batch_test in batches:
                ...
                batch_predicted_labels_ts, batch_predicted_scores_ts = \
                    feed.get_label_threshold(scores=batch_scores, threshold=FLAGS.threshold)
                ...
                batch_predicted_onehot_labels_ts = \
                    feed.get_onehot_label_threshold(scores=batch_scores, threshold=FLAGS.threshold)
                ...
                for top_num in range(FLAGS.top_num):
                    batch_predicted_onehot_labels_tk = feed.get_onehot_label_topk(scores=batch_scores, top_num=top_num + 1)
            ...
            feed.create_prediction_file(output_file=SAVE_DIR + "/predictions.json", data_id=test_data.testid,
