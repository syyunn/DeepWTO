...
while not (TRAIN_OR_RESTORE.isalpha() and
           TRAIN_OR_RESTORE.upper() in ['T', 'R']):
    TRAIN_OR_RESTORE = \
        input("✘ The format of your input is illegal, please re-input: ")

logging.info("✔︎ The format of your input is legal, "
...
TRAININGSET_DIR = '../../../data/test_Randolph/Train.json'
VALIDATIONSET_DIR = '../../../data/test_Randolph/Validation.json'
        ...
        with sess.as_default():
            ...
            if FLAGS.train_or_restore == 'R':
                ...
                while not (MODEL.isdigit() and len(MODEL) == 10):
                    MODEL = input(
                        "✘ The format of your input is illegal, "
                        "please re-input: ")
                logger.info(
            ...
            if FLAGS.train_or_restore == 'R':
                ...
                saver.save(sess, os.path.join(out_dir,
                                              "embedding",
                                              "embedding.ckpt"))
            ...
            def validation_step(_x_val,
                ...
                for batch_validation in batches_validation:
                    ...
                    step, summaries, scores, cur_loss = sess.run(
                        [fasttext.global_step, validation_summary_op,
                         fasttext.scores, fasttext.loss], feed_dict)
                ...
                _eval_pre_ts = precision_score(
                    ...
                    average='micro')
                _eval_rec_ts = recall_score(y_true=np.array(true_onehot_labels),
                                            y_pred=np.array(
                                               predicted_onehot_labels_ts),
                                            average='micro')
                _eval_F_ts = f1_score(y_true=np.array(true_onehot_labels),
                                      y_pred=np.array(
                                         predicted_onehot_labels_ts),
                                      average='micro')
                ...
                _eval_auc = roc_auc_score(y_true=np.array(true_onehot_labels),
                                         y_score=np.array(
                                             predicted_onehot_scores),
                                         average='micro')
